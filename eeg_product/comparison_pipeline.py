...
The known baseline chain for ZuCo EEG-to-text decoding is:
  ...
  V9+QML → V8 + HierarchicalTemporalPooling + QuantumFusionProjector (4-qubit VQC)
        Values provided in submitted_data below.
...
Your job: compare {submission.model_name} vs V9+QML (our reference model) on ZuCo.
...
After the per-metric table:
  OVERALL VERDICT: BEATS V9+QML / MATCHES V9+QML / BELOW V9+QML / INSUFFICIENT DATA
...
Write 4 paragraphs, no headers, no bullets, max 400 words:
  ...
  Para 2: How it compares to V9+QML — where it wins, where it loses, what the numbers mean in plain terms
...
async def run_comparison_pipeline(
    ...
    # 2. Load V9+QML baseline (from results JSON or hardcoded)
    ...
    if verbose:
        ...
        print(f"  Reference       : V9+QML (V5/V8 also in context)")
...
V9+QML REFERENCE:
  BLEU-1={v9qml.get('tf_bleu1_pct')}%  ROUGE-1={v9qml.get('tf_rouge1_pct')}%
  BERTScore={v9qml.get('bertscore_f1')}%  FG={v9qml.get('fg_bleu1_pct')}%
  TF/FG ratio={v9qml.get('tf_fg_ratio')}×
...
METRICS HEADLINE:
  ...
  V9+QML:                  BLEU-1={v9qml.get('tf_bleu1_pct')}%  ROUGE-1={v9qml.get('tf_rouge1_pct')}%
  Delta BLEU-1: {comparison_data['deltas_vs_v9qml']['bleu1']:+}pp
  Delta ROUGE-1: {comparison_data['deltas_vs_v9qml']['rouge1']:+}pp
